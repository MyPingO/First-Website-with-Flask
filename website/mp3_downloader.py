...
def downloader(url: str, downloads_path: str):
    if "/playlist?list=" in url:
        ...
        os.startfile(downloads_path) # open the downloads_path automatically for user convenience
        return
    else:
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ...
            while True:
                ...
                try:
                    ...
                    audio_data = BytesIO()
                    something = YouTube(url)
                    something.streams.get_audio_only().stream_to_buffer(audio_data)
                    audio_data.seek(0)
                    print("Download is complete!")
                    return send_file(audio_data, as_attachment=True, download_name=f"{something.title}.mp3")
